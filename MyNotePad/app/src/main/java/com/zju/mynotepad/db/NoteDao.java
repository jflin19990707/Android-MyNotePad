...
public class NoteDao {
    ...
    public List<Note> queryNotesAll(int groupId) {
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                note.setUrl(cursor.getString(cursor.getColumnIndex("n_url")));
                noteList.add(note);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public long insertNote(Note note) {
        ...
        String sql = "insert into db_note(n_title,n_content,n_group_id,n_group_name," +
                "n_type,n_bg_color,n_encrypt,n_create_time,n_update_time,n_url) " +
                "values(?,?,?,?,?,?,?,?,?,?)";
        ...
        try {
            ...
            stat.bindString(10,note.getUrl());
